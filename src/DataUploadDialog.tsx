...
export default function DataUploadDialog({ instanceid }: Props) {
  ...
  async function selectForUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (e.target.files) {
      ...
      selectedFiles.forEach((file) => {
        ...
        const modifiedPath = pathParts.slice(1).join("/"); // Remove parent directory name
  ...
  async function uploadConfirm() {
    ...
    if (record) {
      ...
      await fetch(import.meta.env.VITE_API_URL + "data/" + params.id, {
        ...
        .then(() => alert("Your database is updated!"))
        .catch((err) => console.log(err));
    }
    setLoading(false);
  }
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Upload pb_data</DialogTitle>
          <DialogDescription>
            Continue where you left off. Upload your pb_data directory.
          </DialogDescription>
        </DialogHeader>
        <Input
          type="file"
